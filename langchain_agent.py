...
class AUBStudentAgent:
    ...
    def _create_tools(self) -> List[Tool]:
        ...
        def llm_sql_tool(query: str) -> str:
            ...
            try:
                ...
                Table: faq (id INTEGER PRIMARY KEY, question TEXT, answer TEXT, program TEXT, embedding BLOB)
                User question: {query}
    ...
    def _create_agent(self):
        ...
        system_prompt = """You are a helpful AUB (American University of Beirut) student assistant. You help students with questions about AUB programs using the available tools. If you cannot use a tool, be friendly and remind the user that you are here to help with AUB program information, and encourage them to ask about AUB programs, admissions, or courses.

INSTRUCTIONS:
1. Always use tools to get factual information about AUB programs
2. For specific program questions, use the semantic_search tool
3. For complex queries requiring structured data, use the llm_sql_tool
4. Always base your answers on the information retrieved from tools
5. Be conversational, helpful, and reference previous conversation when relevant
6. If no relevant information is found, suggest contacting AUB admissions directly via this email: info@aub.edu.lb
7. If you cannot use a tool, respond in a friendly manner and encourage questions about AUB programs, but not all the time.
...
Remember to maintain conversation context and provide helpful, accurate responses based on the tool outputs."""
    ...
    def process_query(self, 
        ...
        try:
            ...
            if selected_program:
                enhanced_input += f"\n[User is focused on: {selected_program}]"
...
def process_user_query(agent: AUBStudentAgent, 
    ...
    """Process user query using the agent"""
    return agent.process_query(user_input, chat_history, selected_program, streamlit_callback, verbose)
